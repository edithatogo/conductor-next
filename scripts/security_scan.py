            ...
            if result['stdout']:
                try:
                    ...
                    results[py_dir] = {
                        ...
                        'severity_counts': data.get('results', []) and {
...
def generate_report(npm_results: Dict[str, dict], python_results: list, 
                   bandit_results: dict, output_file: Path, format: str = 'json'):
    """
    ...
    report = {
        ...
        'summary': {
            'total_npm_vulnerabilities': sum(
                r.get('vulnerabilities', 0) for r in npm_results.values()
            ),
            'total_python_vulnerabilities': len(python_results) if isinstance(python_results, list) else 0,
            'total_bandit_issues': sum(
                r.get('issues', 0) for r in bandit_results.values()
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Output file (default: .conductor/security-report.json)'
    )
    ...
    parser.add_argument(
        '--no-bandit',
        action='store_true',
        help='Skip bandit security lint'
    )
    ...
    total_vulns = (
        report['summary']['total_npm_vulnerabilities'] +
        report['summary']['total_python_vulnerabilities']
    )
