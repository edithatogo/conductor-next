...
def collect_prs(repo: str, output_file: Path) -> dict:
    ...
    # Fetch open PRs
    open_prs = run_gh_command(f"pr list --state open --limit 100", repo)
...
def collect_issues(repo: str, output_file: Path) -> dict:
    ...
    # Fetch open issues
    open_issues = run_gh_command(f"issue list --state open --limit 100", repo)
    ...
    p0_p1_issues = sum(
        ...
        if any(label.get('name', '').lower() in ['p0', 'p1'] 
               for label in issue.get('labels', []))
    )
    
    return {
        'open_total': len(open_issues),
        'high_priority': p0_p1_issues
    }
...
def collect_security_data(output_file: Path) -> dict:
    ...
    return {
        ...
        'python_vulnerabilities': len(security_data['python_safety']) if isinstance(security_data['python_safety'], list) else 0
    }
...
def main():
    ...
    parser.add_argument(
        ...
        help='Repository to analyze (default: edithatogo/conductor-next)'
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Output directory (default: .conductor/improvement-data/)'
    )
    ...
    print(f"🚀 Starting improvement data collection...")
    ...
    try:
        ...
        if not args.no_security:
            ...
            print(f"  ✅ Security scans complete")
        else:
        ...
        if not args.no_security:
            print(f"   Security: {stats['security'].get('npm_vulnerabilities', 0)} NPM, {stats['security'].get('python_vulnerabilities', 0)} Python vulnerabilities")
        print()
